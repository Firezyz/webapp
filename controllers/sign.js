...
exports.signup = function (req, res, next) {
    ...
    // 验证信息的正确性
    if ([loginname, pass, email].some(function (item) {
            return item === '';
        })) {
    ...
    User.isUsernameOrEmailExist(loginname, email, function (users) {
        if (users.length > 0) {
        ...
        tools.bcrypt_hash(pass, ep.done(function (passhash) {
            // create gravatar
            User.newAndSave(loginname, passhash, email, function (err) {
                if (err) {
                    return next(err);
                }
                // 发送激活邮件
...
exports.login = function (req, res, next) {
    ...
    if ([loginname, password].some(function (item) {
            return item === '';
        })) {
        res.status(422);
    ...
    getUser(loginname, function (user) {
        if (!user) {
            return ep.emit('login_failed');
        }
        var passhash = user.password;
        tools.bcrypt_compare(password, passhash, ep.done(function (flag) {
            if (!flag) {
                return ep.emit('login_failed');
            }
